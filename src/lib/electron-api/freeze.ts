...
export type { FreezeAPI, FreezeSnapshot, FreezeState, TextLayer };
...
export function scrollFrozenTab(tabId: string, deltaX: number, deltaY: number): void {
  const api = requireElectron();
  api.freeze.scroll({ tabId, deltaX, deltaY });
}
